...
#-----------------------------------------------------------------------------
# Función: Corregir fechas en tabla operaciones
#-----------------------------------------------------------------------------
def fix_dates(conn, cursor):
    try:
        cursor.execute("""
            SELECT id_registro, fecha FROM operaciones
            WHERE fecha !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$';
        """)
        rows = cursor.fetchall()
        
        if not rows:
            print("No hay fechas para corregir")
            return
        
        correcciones = 0
        for id_registro, fecha in rows:
            try:
                fixed = None

                match = re.match(r'^(\d{2})-(\d{2})-(\d{2})$', fecha)
                if match:
                    anio, dia, mes = match.groups()
                    anio_completo = "20" + anio
                    fixed = f"{anio_completo}-{mes.zfill(2)}-{dia.zfill(2)}"
                
                if not fixed:
                    fixed = parser.parse(fecha).strftime("%Y-%m-%d")
                ...
                print(f"No se pudo parsear: {fecha} - Error: {e}")
        
        conn.commit()
        print(f"Se corrigieron {correcciones} fechas")
        
    except (Exception, Error) as error:
        print("Error corrigiendo fechas:", error)
...
try:
   ...
   # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # CORRECCIÓN DE FECHAS EN TABLA OPERACIONES
    # -------------------------------------------------------------------------
    print("CORRECCIÓN DE FECHAS")
    fix_dates(connection, cursor)
    # -------------------------------------------------------------------------
